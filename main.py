    """FastAPI backend for Glassport Gen portfolio and CV generation."""
    ...
    # Why each skill matters in the current market — used by deterministic fallback
    ...
    def _find_best_role_and_gaps(
        ...
        ideal_stack: list[str] = ROLE_STACKS[best_role]
        missing: list[str] = [
            s for s in ideal_stack if s.lower() not in user_stack_lower
        ]
    ...
    _gemini_model: genai.GenerativeModel | None = (
        genai.GenerativeModel(GEMINI_MODEL_NAME) if _gemini_api_key else None
    )
    ...
    class AnalyticsTracker:
        ...
        def get_stats(self) -> dict[str, int]:
            now = time.time()
            # Clean up old active users (inactive for > 5 mins)
            self.active_users = {
                ip: ts for ip, ts in self.active_users.items() 
                if now - ts < 300
            }
            return {
                "active": len(self.active_users),
                "total": len(self.total_unique_ips)
            }
    ...
    async def lifespan(app: FastAPI) -> Any:
        """Create and clean up shared async resources."""
    ...
    app = FastAPI(title="Glassport Gen Backend", version="1.0.0", lifespan=lifespan)
    ...
    async def track_visitors(request: Request, call_next):
        # Track unique IP on every API request
    ...
    def _build_ai_prompt(
        ...
        context: str = f"Description: {normalized_description}"
        if readme_content:
            # Truncate readme to avoid hitting context limits, though Pro can handle a lot.
            # 4000 characters is usually enough for a high-quality summary.
        ...
        if detailed:
            return (
                ...
                f"Primary Language: {language_label}\n"
                f"{context}"
            )
        return (
        ...
        if description and description.strip():
            if detailed:
                return (
                    ...
                    f"delivered production-ready features, improved reliability, and strengthened developer workflow through "
                    f"structured implementation and maintainable code quality."
                )
    ...
    async def _rewrite_repo_description(
        ...
        fallback: str = _fallback_description(description, repo_name, language, detailed=detailed)
        if _gemini_model is None:
            return fallback

        prompt: str = _build_ai_prompt(
            description, repo_name, language, detailed=detailed, readme_content=readme_content
        )
        try:
            response = await asyncio.to_thread(_gemini_model.generate_content, prompt)
            text: str = (response.text or "").strip() if response else ""
    ...
    async def _fetch_repo_readme(
        ...
        """Fetch the README content for a repository and decode it."""
        try:
    ...
    def _build_market_insight_prompt(
        ...
        return (
            ...
            '"market_skill_ratings": [{"skill": string, "score": integer(1-10)}], '
            ...
            "1. Analyze the user's ACTUAL tech_stack and compare it against what the CURRENT market demands for their best-fit role.\n"
            "2. Identify exactly 2-3 high-demand tools they are GENUINELY missing (not skills they already have).\n"
            "3. The 'why' for each skill must reference real market demand (e.g., '72% of backend roles now require Docker').\n"
    ...
    def _fallback_market_insights(
        ...
        """Generate deterministic market insights if Gemini is unavailable."""
        total_stars: int = sum(int(repo.get("stargazers_count", 0)) for repo in repos)
        repo_count: int = len(repos)
        
        # More scientific probability calculation
        # Base: 30. Projects: +3 per repo (max 15). Stars: log-based bonus. Tech: +2 per unique tech (max 20).
        import math
        ...
        selection_probability: int = max(10, min(base_score, 92))
        target_score = min(98, selection_probability + 12)

        top_skills = tech_stack[:6] if tech_stack else ["Software Engineering", "Git", "API Design"]
        skill_ratings = [
        ...
        return {
            ...
            "strengths": [
                ...
                "Clear stack specialization visible through repository language trends.",
            ],
            "gaps": [
    ...
    def _build_dynamic_career_growth(
        ...
        """Build career_growth dynamically by comparing the user's stack against market demands."""
        best_role, missing_skills = _find_best_role_and_gaps(tech_stack)

        # Pick the top 2-3 missing skills and provide market-aware "why" descriptions
        recommended: list[dict[str, str]] = []
        for skill_name in missing_skills[:3]:
            why = SKILL_WHY.get(skill_name, f"High-demand skill for {best_role} roles in the current market.")
            recommended.append({"skill": skill_name, "why": why})

        # If no skills are missing from their best role, suggest cross-discipline skills
        if not recommended:
            cross_skills = ["Docker", "AWS", "Kubernetes"]
            user_lower = {s.lower() for s in tech_stack}
            for s in cross_skills:
    ...
    def _rank_top_original_repos(
        ...
        filtered: list[dict[str, Any]] = _filter_original_repos(repos, username)
        ranked: list[dict[str, Any]] = sorted(
            filtered, key=lambda repo: int(repo.get("stargazers_count", 0)), reverse=True
        )
        return ranked[:limit]
    ...
    async def _build_projects_with_ai(
        ...
        if detailed:
            for repo in top_repos:
                ...
                readme_tasks.append(_fetch_repo_readme(client, username, repo_name))
            ...
            tasks.append(
                asyncio.create_task(
                    _rewrite_repo_description(
                        ...
                        repo_name=str(repo.get("name") or "Untitled Project"),
                        language=repo.get("language"),
                        detailed=detailed,
        ...
        rewritten_descriptions: list[str] = await asyncio.gather(*tasks)

        projects: list[dict[str, Any]] = []
        for repo, ai_description in zip(top_repos, rewritten_descriptions):
            projects.append(
                {
                    "title": repo.get("name"),
    ...
    async def _get_portfolio_payload(
        ...
        client: httpx.AsyncClient = app.state.http_client
        profile_task = asyncio.create_task(_fetch_github_profile(client, username))
        repos_task = asyncio.create_task(_fetch_github_repos(client, username))

        profile, repos = await asyncio.gather(profile_task, repos_task)
        all_original_repos: list[dict[str, Any]] = _rank_top_original_repos(
            repos, username, limit=10_000
        )
        total_projects: int = len(all_original_repos)
        start_idx: int = (page - 1) * page_size
        end_idx: int = start_idx + page_size
        paginated_repos: list[dict[str, Any]] = all_original_repos[start_idx:end_idx]
        projects_task = asyncio.create_task(_build_projects_with_ai(username, paginated_repos))
        market_insights_task = asyncio.create_task(
            _generate_market_insights(
                profile=profile,
                repos=all_original_repos,
                tech_stack=_extract_tech_stack(repos),
            )
        )
        projects, market_insights = await asyncio.gather(projects_task, market_insights_task)
        total_pages: int = max(1, (total_projects + page_size - 1) // page_size)
    ...
    def _build_resume_pdf(portfolio: dict[str, Any], username: str) -> bytes:
        """Generate a polished, professional PDF resume in memory from portfolio data."""
        ...
        name: str = str(user.get("name") or username)
        github_url: str = str(user.get("github_url") or f"https://github.com/{username}")
        location: str = str(user.get("location") or "Location not specified")
        ...
        if prof_summary:
            pdf.set_draw_color(0, 102, 204)  # Subtle blue accent
        ...
        if tech_stack:
            ...
            skills_text: str = ", ".join(tech_stack[:15])
            pdf.set_font("Helvetica", "", 11)
            pdf.set_text_color(60, 60, 60)
        ...
        for project in top_projects:
            title: str = str(project.get("title") or "Untitled Project")
            language: str = str(project.get("language") or "Unknown")
            ai_description: str = str(project.get("ai_description") or "")
            stars: int = int(project.get("stars") or 0)
            project_url: str = str(project.get("url") or "")

            # Clean description
            safe_ai_description: str = " ".join(ai_description.split())

            # Project Header
            pdf.set_text_color(20, 20, 20)
            pdf.set_font("Helvetica", "B", 12)
            pdf.cell(pdf.w - pdf.l_margin - pdf.r_margin - 40, 7, f"{title}")
            ...
            # Use a bullet point with hanging indent if possible, or just multi_cell
            pdf.set_x(pdf.l_margin + 3)
            pdf.multi_cell(0, 6, f"- {safe_ai_description}")
            pdf.ln(2.5)
        ...
        pdf.cell(
            0,
            5,
            "Generated by Glassport Premium AI Resume Engine",
            align="C",
        )
    ...
    async def get_portfolio(
        ...
        try:
            return await _get_portfolio_payload(
                username=username,
                page=page,
                page_size=page_size,
            )
    ...
    # The Catch-All: This sends the user to your React UI
    @app.get("/{catchall:path}")
    async def serve_frontend(catchall: str):
        # If the request starts with /api, we don't want to serve HTML
        ...
        return {"error": "Frontend build not found. Ensure 'npm run build' completed."}
    ...
    async def get_cv(username: str) -> StreamingResponse:
        ...
        try:
            ...
            repos_task = asyncio.create_task(_fetch_github_repos(client, username))
            profile, repos = await asyncio.gather(profile_task, repos_task)
            top_repos_for_cv: list[dict[str, Any]] = _rank_top_original_repos(
                repos, username, limit=TOP_CV_PROJECTS
            )
            cv_projects: list[dict[str, Any]] = await _build_projects_with_ai(
                username, top_repos_for_cv, detailed=True
            )
            ...
            # Generate the professional summary using the enhanced AI logic
            portfolio["professional_summary"] = await _generate_professional_summary(
                portfolio, portfolio["tech_stack"]
            )
            pdf_bytes: bytes = _build_resume_pdf(portfolio=portfolio, username=username)
        except HTTPException:
            raise
        except httpx.HTTPStatusError as exc:
            raise HTTPException(status_code=exc.response.status_code, detail="GitHub API error.")
    ...
    # ─── Serve React build (production: all-in-one container) ───────────
